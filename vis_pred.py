...
if __name__ == '__main__':
    ...
    lut1 = LabelLUT() # to be modified
    for val in sorted(dataset.label_to_names.keys()):
        lut1.add_label(val, val)

    vis.visualize([{
        "name": "KITTI",
        'points': data['point']
    }], lut=lut1, bounding_boxes = result)
    
    #dataset.get_split('training').get_data(1)
    #vis.visualize_dataset(dataset, 'training', indices=range(1000))
